...
conf_matrix_after = [[5603, 1368], [328, 582]]
...
sns.heatmap(conf_matrix_after, annot=True, fmt='d', cmap='Blues', xticklabels=categories, yticklabels=categories)
plt.title('Random Forrest Confusion Matrix - After Handling Imbalanced Classes')
...
categories = ['0', '1']
plt.figure(figsize=(8, 6))
sns.heatmap(conf_matrix_best, annot=True, fmt='d', cmap='Blues', xticklabels=categories, yticklabels=categories)
...
# Plot Confusion Matrix - After Handling
plt.subplot(1, 2, 2)
sns.heatmap(conf_matrix_after_svm, annot=True, fmt='d', cmap='Blues', xticklabels=categories, yticklabels=categories)
plt.title('SVM Confusion Matrix - After Handling')
plt.xlabel('Predicted')
plt.ylabel('True')
...
# Confusion Matrix Data
conf_matrix_after_nn = [[5893, 1070], [363, 555]]
...
# Plot Confusion Matrix
plt.figure(figsize=(8, 6))
sns.heatmap(conf_matrix_after_nn, annot=True, fmt='d', cmap='Blues', xticklabels=categories, yticklabels=categories)
plt.title('Confusion Matrix - After Handling Imbalanced Classes with Neural Network')
plt.xlabel('Predicted')
plt.ylabel('True')
plt.show()
...
predictions = [y_pred, y_pred_svm, y_pred_after_lr, y_pred_nn_after]
probabilities = [rf_classifier.predict_proba(X_test)[:, 1], svm_model.decision_function(X_test_scaled), logistic_regression_model_after.decision_function(X_test_scaled), model_after.predict(X_test_scaled).flatten()]

# Plot ROC curve for each model
plt.figure(figsize=(10, 6))
for model, prediction, probability in zip(models, predictions, probabilities):
